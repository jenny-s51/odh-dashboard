...
import { GraphElement, RunStatus, observer, useHover, useSize, useCombineRefs, useDetailsLevel, useAnchor, TaskNodeSourceAnchor, AnchorEnd, TaskNodeTargetAnchor, ScaleDetailsLevel, useScaleNode, getNodeScaleTranslation, getRunStatusModifier, createSvgIdUrl, nonShadowModifiers, LabelIcon, LabelBadge, StatusIcon, NodeShadows, LabelActionIcon, LabelContextMenu } from "@patternfly/react-topology";
import { NODE_SHADOW_FILTER_ID_DANGER, NODE_SHADOW_FILTER_ID_HOVER } from "@patternfly/react-topology/dist/esm/components/nodes/NodeShadows";
...
const TaskNodeInner: React.FC<TaskNodeInnerProps> = observer(({
  ...
  const nameLabel = (
    <text ref={nameLabelRef} className={css(topologyPipelinesPillText)} dominantBaseline="middle">
      {label}
    </text>
  );
