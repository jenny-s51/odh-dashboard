...
const TaskNodeInner: React.FC<TaskNodeInnerProps> = observer(
  ({
    ...
    useAnchor(
      React.useCallback(
        (node: Node) =>
          new TaskNodeTargetAnchor(node, hasWhenExpression ? 0 : whenSize + whenOffset),
    ...
    React.useEffect(() => {
      action(() => {
        ...
        sourceEdges.forEach((edge) => {
          ...
          edge.setData({
            ...(data || {}),
            indent: detailsLevel === ScaleDetailsLevel.high ? width - pillWidth : 0,
          });
        });
      })();
    ...
    const nameLabel = (
      <text
        ref={nameLabelRef}
        className={css(styles.topologyPipelinesPillText)}
      ...
      if (
        ...
        return (
          ...
          >
            ...
            />
            {status &&
            (!hiddenDetailsShownStatuses || hiddenDetailsShownStatuses.includes(status)) ? (
      ...
      return (
        ...
        >
          ...
          {status && showStatusState && (
            <g
              transform={`translate(${statusStartX + paddingX / 2}, ${
                (height - statusIconSize) / 2
              })`}
              ref={statusRef}
            >
              <g
          ...
          {leadIcon && (
            <g
              transform={`translate(${leadIconStartX}, ${(height - leadSize?.height ?? 0) / 2})`}
              ref={leadIconRef}
            >
              {leadIcon}
            </g>
    ...
    return (
      <g
        ...
        transform={`${
          scaleNode ? `translate(${translateX}, ${translateY})` : ''
        } scale(${nodeScale})`}
        ref={hoverRef}
      >
